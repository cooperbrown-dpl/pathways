...
namespace EFDemo
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("People who live in Springifeld:");
            ...
            var numOfContacts = context.Contacts.Count();
            var numOfAddresses = context.Addresses.Count();

            Console.WriteLine($"# of Contacts: {numOfContacts}, # of Addresses: {numOfAddresses}");

            //Activity 7
            //Task: Show the names of all contacts that have at least one address in "Shelbyville".
            Console.WriteLine("----- Activity 7 -----");
            var contactsWithShelbyvilleAddress = context.Contacts
                .Where(c => c.Addresses.Any(a => a.City == "Shelbyville"))
                .Select(c => c.Name)
                .ToList();
            Console.WriteLine("People who have an address in Shelbyville");

            foreach (var name in contactsWithShelbyvilleAddress)
            {
                Console.WriteLine($"- {name}");
            }
            ...
            var sortedAddresses = context.Addresses
                ...
                .ThenBy(a => a.Street)
                .ToList();
            Console.WriteLine("Addresses sorted alphabetically by city, then by street.");
            ...
            var contactWithMostAddresses = context.Contacts
                .Include(c => c.Addresses)
                .OrderByDescending(c => c.Addresses.Count())
                .FirstOrDefault();

            if (contactWithMostAddresses != null)
            {
                Console.WriteLine($"{contactWithMostAddresses.Name} has the most addresses.");
                Console.WriteLine($"Number of addresses: {contactWithMostAddresses.Addresses.Count()}");
            }

            //Activity 10
            //Task: Show names of contacts who have NO address in "Springfield".
            Console.WriteLine("----- Activity 10 -----");
            var noAddressInSpringfield = context.Contacts
                .Where(c => !c.Addresses.Any(a => a.City == "Springfield"))
                .Select(c => c.Name)
                .ToList();

            foreach (var contact in noAddressInSpringfield)
            {
